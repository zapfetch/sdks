...
from zapfetch.v2.types import ScrapeOptions
...
async def test_async_crawl_start_and_status():
    client = AsyncZapfetch(api_key=os.getenv("API_KEY"), api_url=os.getenv("API_URL"))
    start = await client.start_crawl("https://docs.zapfetch.com", limit=2)
    job_id = start.id
...
async def test_async_start_crawl_with_options():
    client = AsyncZapfetch(api_key=os.getenv("API_KEY"), api_url=os.getenv("API_URL"))
    start = await client.start_crawl("https://docs.zapfetch.com", limit=5, max_discovery_depth=2)
...
async def test_async_start_crawl_with_prompt():
    client = AsyncZapfetch(api_key=os.getenv("API_KEY"), api_url=os.getenv("API_URL"))
    start = await client.start_crawl("https://zapfetch.com", prompt="Extract all blog posts", limit=3)
    assert start.id is not None and start.url is not None
...
async def test_async_get_crawl_status_shape():
    client = AsyncZapfetch(api_key=os.getenv("API_KEY"), api_url=os.getenv("API_URL"))
    start = await client.start_crawl("https://docs.zapfetch.com", limit=3)
...
async def test_async_crawl_with_wait():
    client = AsyncZapfetch(api_key=os.getenv("API_KEY"), api_url=os.getenv("API_URL"))
    job = await client.crawl(url="https://docs.zapfetch.com", limit=3, max_discovery_depth=2, poll_interval=1, timeout=120)
...
async def test_async_crawl_with_prompt_and_wait():
    client = AsyncZapfetch(api_key=os.getenv("API_KEY"), api_url=os.getenv("API_URL"))
    job = await client.crawl(url="https://docs.zapfetch.com", prompt="Extract all blog posts", limit=3, poll_interval=1, timeout=120)
    assert job.status in ("completed", "failed")
    assert job.completed >= 0 and job.total >= 0 and isinstance(job.data, list)
...
async def test_async_crawl_with_scrape_options():
    ...
    start = await client.start_crawl("https://docs.zapfetch.com", limit=2, scrape_options=scrape_opts)
