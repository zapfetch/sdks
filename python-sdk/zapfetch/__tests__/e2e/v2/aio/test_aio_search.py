...
zapfetch = AsyncZapfetch(api_key=os.getenv("API_KEY"), api_url=os.getenv("API_URL"))
...
async def test_async_search_all_parameters():
    from zapfetch.types import ScrapeOptions, Location, WaitAction
    schema = {
        "type": "object",
        "properties": {
            "title": {"type": "string"},
...
async def test_async_search_formats_flexibility():
    ...
    results1 = await zapfetch.search(
    ...
    results2 = await zapfetch.search(
        query="python programming",
        limit=1,
        scrape_options=ScrapeOptions(
