...
def api_url():
    # Prefer TEST_URL, then ZAPFETCH_API_URL (for parity with JS), then legacy API_URL
    return (
        os.getenv("TEST_URL")
        or os.getenv("ZAPFETCH_API_URL")
        or os.getenv("API_URL")
        or "https://api.zapfetch.com"
    )
...
def _inject_client(request, api_identity, api_url):
    ...
    if inst is not None and not hasattr(inst, "client"):
        try:
            ...
            inst.client = Zapfetch(api_key=api_identity.get("apiKey", ""), api_url=api_url)
        except Exception:
            pass
    # For function-based modules that expect a module-level `zapfetch` symbol
    mod = getattr(request, "module", None)
    if mod is not None and not hasattr(mod, "zapfetch"):
        try:
            from zapfetch import Zapfetch
            setattr(mod, "zapfetch", Zapfetch(api_key=api_identity.get("apiKey", ""), api_url=api_url))
        except Exception:
            pass
