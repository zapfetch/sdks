...
from zapfetch.v2.utils.http_client_async import AsyncHttpClient
from zapfetch.v2.utils.http_client import HttpClient
from zapfetch.v2.methods.aio import batch as aio_batch
...
async def test_scrape_concurrency(monkeypatch):
    ...
    client = AsyncZapfetchClient(api_key="test", api_url="http://localhost")

    start = time.perf_counter()
    await asyncio.gather(
        client.scrape("https://zapfetch.com"),
        client.scrape("https://zapfetch.com"),
        client.scrape("https://zapfetch.com")
    )
