"""
Unit tests for Zapfetch v2 pagination functionality.
"""
...
from zapfetch.v2.methods.crawl import get_crawl_status, get_crawl_status_page, _fetch_all_pages
from zapfetch.v2.methods.batch import get_batch_scrape_status, get_batch_scrape_status_page, _fetch_all_batch_pages
from zapfetch.v2.methods.aio.crawl import (
    get_crawl_status as get_crawl_status_async,
    get_crawl_status_page as get_crawl_status_page_async,
    _fetch_all_pages_async,
)
from zapfetch.v2.methods.aio.batch import (
...
class TestCrawlPagination:
    ...
    def test_get_crawl_status_no_pagination(self):
        ...
        mock_response.json.return_value = {
            ...
            "next": "https://api.zapfetch.com/v2/crawl/test-crawl-123?page=2",
        ...
        assert result.next == "https://api.zapfetch.com/v2/crawl/test-crawl-123?page=2"
    ...
    def test_get_crawl_status_propagates_request_timeout(self):
        ...
        timeout_seconds = 5.5
        import zapfetch.v2.methods.crawl as crawl_module

        assert crawl_module.__file__.endswith("zapfetch/v2/methods/crawl.py")
    ...
    def test_get_crawl_status_page(self):
        ...
        mock_response.json.return_value = {
            ...
            "creditsUsed": 5,
            "expiresAt": "2024-01-01T00:00:00Z",
            "next": "https://api.zapfetch.com/v2/crawl/test-crawl-123?page=3",
            "data": [self.sample_doc],
        }

        self.mock_client.get.return_value = mock_response
        next_url = "https://api.zapfetch.com/v2/crawl/test-crawl-123?page=2"

        result = get_crawl_status_page(self.mock_client, next_url)

        assert result.status == "completed"
        assert result.next == "https://api.zapfetch.com/v2/crawl/test-crawl-123?page=3"
        assert len(result.data) == 1
    ...
    def test_get_crawl_status_max_pages_limit(self):
        ...
        for i in range(5):  # 5 pages available
            ...
            mock_response.json.return_value = {
                ...
                "creditsUsed": (i + 1) * 5,
                "expiresAt": "2024-01-01T00:00:00Z",
                "next": f"https://api.zapfetch.com/v2/crawl/test-crawl-123?page={i+2}" if i < 4 else None,
                "data": [self.sample_doc]
            }
    ...
    def test_get_crawl_status_max_wait_time_limit(self):
        ...
        with patch('zapfetch.v2.methods.crawl.time.monotonic', side_effect=[0, 2]):  # Simulate 2 seconds elapsed
            result = get_crawl_status(self.mock_client, self.job_id, pagination_config)
    ...
    def test_fetch_all_pages_error_handling(self):
        ...
        result = _fetch_all_pages(self.mock_client, "https://api.zapfetch.com/v2/crawl/test-crawl-123?page=2", [], None)
...
class TestBatchScrapePagination:
    ...
    def test_get_batch_scrape_status_no_pagination(self):
        ...
        mock_response.json.return_value = {
            ...
            "creditsUsed": 5,
            "expiresAt": "2024-01-01T00:00:00Z",
            "next": "https://api.zapfetch.com/v2/batch/scrape/test-batch-123?page=2",
            "data": [self.sample_doc]
        }
        ...
        assert result.status == "completed"
        assert result.next == "https://api.zapfetch.com/v2/batch/scrape/test-batch-123?page=2"
        assert len(result.data) == 1
        assert isinstance(result.data[0], Document)
    ...
    def test_get_batch_scrape_status_page(self):
        ...
        mock_response.json.return_value = {
            ...
            "creditsUsed": 5,
            "expiresAt": "2024-01-01T00:00:00Z",
            "next": "https://api.zapfetch.com/v2/batch/scrape/test-batch-123?page=3",
            "data": [self.sample_doc],
        }

        self.mock_client.get.return_value = mock_response
        next_url = "https://api.zapfetch.com/v2/batch/scrape/test-batch-123?page=2"

        result = get_batch_scrape_status_page(self.mock_client, next_url)

        assert result.status == "completed"
        assert result.next == "https://api.zapfetch.com/v2/batch/scrape/test-batch-123?page=3"
        assert len(result.data) == 1
        self.mock_client.get.assert_called_with(next_url, timeout=None)
    ...
    def test_fetch_all_batch_pages_limits(self):
        ...
        for i in range(5):  # 5 pages available
            ...
            mock_response.json.return_value = {
                ...
                "next": f"https://api.zapfetch.com/v2/batch/scrape/test-batch-123?page={i+2}" if i < 4 else None
            }
            mock_responses.append(mock_response)
        ...
        result = _fetch_all_batch_pages(
            self.mock_client, 
            "https://api.zapfetch.com/v2/batch/scrape/test-batch-123?page=2", 
...
class TestAsyncPagination:
    ...
    async def test_get_crawl_status_async_with_pagination(self):
        ...
        mock_response1.json.return_value = {
            ...
            "creditsUsed": 5,
            "expiresAt": "2024-01-01T00:00:00Z",
            "next": "https://api.zapfetch.com/v2/crawl/test-async-123?page=2",
            "data": [self.sample_doc]
        }
    ...
    async def test_get_crawl_status_async_propagates_request_timeout(self):
        ...
        timeout_seconds = 3.3
        import zapfetch.v2.methods.aio.crawl as crawl_module_async

        assert crawl_module_async.__file__.endswith("zapfetch/v2/methods/aio/crawl.py")
    ...
    async def test_get_crawl_status_page_async(self):
        ...
        mock_response.json.return_value = {
            ...
            "creditsUsed": 5,
            "expiresAt": "2024-01-01T00:00:00Z",
            "next": "https://api.zapfetch.com/v2/crawl/test-async-123?page=3",
            "data": [self.sample_doc],
        }

        self.mock_client.get.return_value = mock_response
        next_url = "https://api.zapfetch.com/v2/crawl/test-async-123?page=2"

        result = await get_crawl_status_page_async(self.mock_client, next_url)

        assert result.status == "completed"
        assert result.next == "https://api.zapfetch.com/v2/crawl/test-async-123?page=3"
        assert len(result.data) == 1
    ...
    async def test_get_batch_scrape_status_async_with_pagination(self):
        ...
        mock_response1.json.return_value = {
            ...
            "creditsUsed": 5,
            "expiresAt": "2024-01-01T00:00:00Z",
            "next": "https://api.zapfetch.com/v2/batch/scrape/test-async-123?page=2",
            "data": [self.sample_doc]
        }
    ...
    async def test_get_batch_scrape_status_page_async(self):
        ...
        mock_response.json.return_value = {
            ...
            "creditsUsed": 5,
            "expiresAt": "2024-01-01T00:00:00Z",
            "next": "https://api.zapfetch.com/v2/batch/scrape/test-async-123?page=3",
            "data": [self.sample_doc],
        }

        self.mock_client.get.return_value = mock_response
        next_url = "https://api.zapfetch.com/v2/batch/scrape/test-async-123?page=2"

        result = await get_batch_scrape_status_page_async(self.mock_client, next_url)

        assert result.status == "completed"
        assert result.next == "https://api.zapfetch.com/v2/batch/scrape/test-async-123?page=3"
        assert len(result.data) == 1
        self.mock_client.get.assert_awaited_with(next_url, timeout=None)
    ...
    async def test_fetch_all_pages_async_limits(self):
        ...
        for i in range(3):  # 3 pages available
            ...
            mock_response.json.return_value = {
                "success": True,
                "data": [self.sample_doc],
                "next": f"https://api.zapfetch.com/v2/crawl/test-async-123?page={i+2}" if i < 2 else None
            }
            mock_responses.append(mock_response)
        ...
        result = await _fetch_all_pages_async(
            self.mock_client,
            "https://api.zapfetch.com/v2/crawl/test-async-123?page=2",
            [Document(**self.sample_doc)],  # 1 initial doc
            pagination_config
        )
...
class TestPaginationEdgeCases:
    ...
    def test_pagination_with_unsuccessful_page(self):
        ...
        mock_response1.json.return_value = {
            "success": True,
            "data": [self.sample_doc],
            "next": "https://api.zapfetch.com/v2/crawl/test-123?page=2"
        }
        ...
        # Should continue with what we have
        result = _fetch_all_pages(self.mock_client, "https://api.zapfetch.com/v2/crawl/test-123?page=2", [], None)
