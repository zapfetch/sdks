...
A Zapfetch client that enables you to scrape content from websites, crawl entire sites, search the web, and extract structured data using AI.
...
Usage:
    ...
    zapfetch = Zapfetch(api_key="your-api-key")
    result = zapfetch.scrape("https://example.com")
...
from .v1 import V1ZapfetchApp, AsyncV1ZapfetchApp
from .v2 import ZapfetchClient as V2ZapfetchClient
from .v2.client_async import AsyncZapfetchClient
from .v2.types import Document

logger = logging.getLogger("zapfetch")

class V1Proxy:
    """Type-annotated proxy for v1 client methods."""
    _client: Optional[V1ZapfetchApp]
    
    def __init__(self, client_instance: Optional[V1ZapfetchApp]):
...
class V2Proxy:
    ...
    _client: Optional[V2ZapfetchClient]
    
    def __init__(self, client_instance: Optional[V2ZapfetchClient]):
        self._client = client_instance
...
class AsyncV1Proxy:
    """Type-annotated proxy for v1 client methods."""
    _client: Optional[AsyncV1ZapfetchApp]
    
    def __init__(self, client_instance: Optional[AsyncV1ZapfetchApp]):
        self._client = client_instance
...
class AsyncV2Proxy:
    """Proxy class that forwards method calls to the appropriate version client."""
    _client: Optional[AsyncZapfetchClient] = None

    def __init__(self, client_instance: Optional[AsyncZapfetchClient] = None):
        self._client = client_instance
...
class Zapfetch:
    """
    Unified Zapfetch client (v2 by default, v1 under ``.v1``).
    ...
    def __init__(
        self,
        api_key: str = None,
        api_url: str = "https://api.zapfetch.com",
        timeout: float = None,
        max_retries: int = 3,
        ...
        Args:
            api_key: Zapfetch API key (or set ``ZAPFETCH_API_KEY``)
        ...
        self._v1_client = V1ZapfetchApp(api_key=api_key, api_url=api_url) if V1ZapfetchApp else None
        self._v2_client = V2ZapfetchClient(
            api_key=api_key,
            api_url=api_url,
            timeout=timeout,
            max_retries=max_retries,
            backoff_factor=backoff_factor,
        ) if V2ZapfetchClient else None
...
class AsyncZapfetch:
    """Async unified Zapfetch client (v2 by default, v1 under ``.v1``)."""
    ...
    def __init__(
        ...
        # Initialize version-specific clients
        self._v1_client = AsyncV1ZapfetchApp(api_key=api_key, api_url=api_url) if AsyncV1ZapfetchApp else None
        self._v2_client = AsyncZapfetchClient(
            api_key=api_key,
            api_url=api_url,
            timeout=timeout,
            max_retries=max_retries,
            backoff_factor=backoff_factor,
        ) if AsyncZapfetchClient else None
...
# Export Zapfetch as an alias for ZapfetchApp
ZapfetchApp = Zapfetch
AsyncZapfetchApp = AsyncZapfetch
