...
class ZapfetchError(Exception):
    """Base exception for Zapfetch API errors."""
...
class BadRequestError(ZapfetchError):
    """Raised when the request is invalid (400)."""
    pass



class UnauthorizedError(ZapfetchError):
    """Raised when the request is unauthorized (401)."""
    pass


class PaymentRequiredError(ZapfetchError):
    """Raised when payment is required (402)."""
    pass


class WebsiteNotSupportedError(ZapfetchError):
    """Raised when website is not supported (403)."""
    pass


class RequestTimeoutError(ZapfetchError):
    """Raised when request times out (408)."""
    pass


class RateLimitError(ZapfetchError):
    """Raised when the rate limit is exceeded (429)."""
    pass


class InternalServerError(ZapfetchError):
...
def handle_response_error(response: requests.Response, action: str) -> None:
    ...
    Raises:
        ZapfetchError: Appropriate error based on status code
    """
    try:
    ...
    if response.status_code == 400:
        ...
        raise ZapfetchError(message, response.status_code, response)