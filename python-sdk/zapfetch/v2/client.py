...
from .utils.error_handler import ZapfetchError
...
class ZapfetchClient:
    """
    Main Zapfetch v2 API client.

    This client provides a clean, modular interface to all Zapfetch functionality.
    """

    @staticmethod
    def _is_cloud_service(url: str) -> bool:
        return "api.zapfetch.com" in url.lower()
    ...
    def __init__(
        ...
        """
        Initialize the Zapfetch client.

        Args:
            api_key: Zapfetch API key (or set ZAPFETCH_API_KEY env var)
            api_url: Base URL for the Zapfetch API
        ...
        if self._is_cloud_service(api_url) and not api_key:
            raise ValueError(
                "API key is required for the cloud API. Set ZAPFETCH_API_KEY environment variable "
