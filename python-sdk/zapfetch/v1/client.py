"""
Zapfetch v1 API Client - Legacy Implementation

This module provides the legacy v1 implementation of the Zapfetch SDK.
It contains the complete `V1ZapfetchApp` class with all v1 API methods and types
...
Classes:
    - V1ZapfetchApp: Legacy v1 client for interacting with the Zapfetch API.
    - AsyncV1ZapfetchApp: Async version of the v1 client.
...
logger : logging.Logger = logging.getLogger("zapfetch")
...
# class V1ZapfetchDocumentMetadata(pydantic.BaseModel):
...
class V1ZapfetchDocument(pydantic.BaseModel, Generic[T]):
    """Document retrieved or processed by Zapfetch."""
...
class V1ScrapeResponse(V1ZapfetchDocument[T], Generic[T]):
...
class V1ZapfetchApp:
    """
    Legacy v1 Zapfetch client for backward compatibility.
    ...
    def __init__(self, api_key: Optional[str] = None, api_url: Optional[str] = None) -> None:
        """
        Initialize the V1ZapfetchApp instance with API key, API URL.

        Args:
            api_key (Optional[str]): API key for authenticating with the Zapfetch API.
            api_url (Optional[str]): Base URL for the Zapfetch API.
        """
        self.api_key = api_key or os.getenv('ZAPFETCH_API_KEY')
        self.api_url = api_url or os.getenv('ZAPFETCH_API_URL', 'https://api.zapfetch.com')
        
        # Only require API key when using cloud service
        if 'api.zapfetch.com' in self.api_url and self.api_key is None:
            logger.warning("No API key provided for cloud service")
            raise ValueError('No API key provided')
            
        logger.debug(f"Initialized V1ZapfetchApp with API URL: {self.api_url}")
    ...
    def scrape_url(
        ...
        if response.status_code == 200:
            try:
                ...
                raise Exception('Failed to parse Zapfetch response as JSON.')
        else:
    ...
    def search(
        ...
        """
        Search for content using Zapfetch.
        ...
        Returns:
            SearchResponse: Response containing:
                ...
                * data (List[ZapfetchDocument]): Search results
        ...
        **kwargs
        ...
        if response.status_code == 200:
            try:
                ...
                raise Exception(f'Failed to parse Zapfetch response as JSON.')
    ...
    def check_batch_scrape_status(self, id: str) -> V1BatchScrapeStatusResponse:
        """
        Check the status of a batch scrape job using the Zapfetch API.
    ...
    def extract(
        ...
        .. deprecated::
            ...
            Review https://docs.zapfetch.com/developer-guides/usage-guides/choosing-the-data-extractor
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    raise Exception(f'Failed to parse Zapfetch response as GenerateLLMsTextStatusResponse: {str(e)}')
...
class V1CrawlWatcher:
    ...
    Attributes:
        ...
        app (V1ZapfetchApp): The V1ZapfetchApp instance
    ...
    """
    def __init__(self, id: str, app: V1ZapfetchApp):
        self.id = id
        self.app = app
...
class AsyncV1ZapfetchApp(V1ZapfetchApp):
    """
    Asynchronous version of V1ZapfetchApp that implements async methods using aiohttp.
    Provides non-blocking alternatives to all V1ZapfetchApp operations.
    """

    def __init__(self, api_key: str, api_url: str = "https://api.zapfetch.com"):
    ...
    async def _handle_error(self, response: aiohttp.ClientResponse, action: str) -> None:
        ...
        try:
            ...
            raise aiohttp.ClientError(f'Failed to parse Zapfetch error response as JSON. Status code: {response.status}')
    ...
    async def search(
            ...
            **kwargs) -> V1SearchResponse:
        """
        Asynchronously search for content using Zapfetch.
...
class AsyncV1CrawlWatcher(V1CrawlWatcher):
    ...
    """
    def __init__(self, id: str, app: AsyncV1ZapfetchApp):
