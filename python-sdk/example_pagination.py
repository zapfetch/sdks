"""
Example demonstrating pagination functionality in Zapfetch v2 SDK.
...
crawl_result = zapfetch.crawl("https://example.com", limit=100)
...
batch_result = zapfetch.batch_scrape(urls)
...
# The next URL is opaque; pass it back to the SDK to fetch the next page.
print("\n=== Example 8: Manual batch scrape with pagination control ===")
batch_job = zapfetch.start_batch_scrape(urls)
status = zapfetch.get_batch_scrape_status(batch_job.id, pagination_config=PaginationConfig(auto_paginate=False))
print(f"Batch scrape documents: {len(status.data)}")
print(f"Next URL: {status.next}")
if status.next:
    next_page = zapfetch.get_batch_scrape_status_page(status.next)
...
async def async_example():
    async_client = AsyncZapfetch(api_key="your-api-key")
