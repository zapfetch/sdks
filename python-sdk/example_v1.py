from zapfetch import V1JsonConfig, ZapfetchApp, AsyncZapfetchApp, V1ScrapeOptions, V1JsonConfig
...
api_url = os.getenv("TEST_API_URL", "https://api.zapfetch.com")

app = ZapfetchApp(api_key=api_key, api_url=api_url)
...
# # # Test batch scrape
urls = ['https://example.com', 'https://docs.zapfetch.com']
...
# # Crawl a website:
crawl_result = app.v1.crawl_url('zapfetch.com', exclude_paths=['blog/*'])
print(crawl_result.data[0].markdown)

# # Asynchronous Crawl a website:
async_result = app.v1.async_crawl_url('zapfetch.com', exclude_paths=['blog/*'])
print(async_result)
...
# Map a website:
map_result = app.v1.map_url('https://zapfetch.com', search="blog")
print(map_result)
...
extract_result = app.v1.extract(['https://zapfetch.com'], prompt="Extract the title, description, and links from the website", schema=extract_schema)
...
async def start_crawl_and_watch():
    ...
    watcher = app.crawl_url_and_watch('zapfetch.com', { 'excludePaths': ['blog/*'], 'limit': 5 })
...
data = app.scrape_url('https://docs.zapfetch.com/', formats=['json'], json_options=extract_config)
print(data.json)

# --- Async

async_app = AsyncZapfetchApp(api_url="https://api.zapfetch.com")
...
async def example_crawl():
    # Crawl a website:
    crawl_result = await async_app.v1.crawl_url('zapfetch.com', exclude_paths=['blog/*'])
    print(crawl_result.data[0].markdown)

    # Asynchronous Crawl a website:
    async_result = await async_app.v1.async_crawl_url('zapfetch.com', exclude_paths=['blog/*'])
    print(async_result)
...
async def example_map_and_extract():
    # Map a website:
    map_result = await async_app.v1.map_url('https://zapfetch.com', search="blog")
    print(map_result)
    ...
    # Perform the extraction
    extract_result = await async_app.v1.extract(['https://zapfetch.com'], prompt="Extract the title, description, and links from the website", schema=extract_schema)
    print(extract_result)
...
async def example_websocket_crawl():
    # Initiate the crawl job and get the watcher
    watcher = await async_app.v1.crawl_url_and_watch('zapfetch.com', { 'excludePaths': ['blog/*'], 'limit': 5 })
