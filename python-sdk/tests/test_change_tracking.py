...
import json
import os
from zapfetch import V1ZapfetchApp

# These tests target an older v1 API that accepted a positional options dict
# (scrape_url(url, {...})); the current v1 client uses kwargs. Skipping until
# the tests are ported to the kwargs-based signature.
@unittest.skip("pending rewrite for current v1 kwargs API")
class TestChangeTracking(unittest.TestCase):
    ...
    def test_change_tracking_format(self, mock_post):
        ...
        app = V1ZapfetchApp(api_key=os.environ.get('TEST_API_KEY', 'dummy-api-key-for-testing'))
