...
describe("v2.crawl e2e", () => {
  ...
  test("start crawl minimal request", async () => {
    ...
    const job = await client.startCrawl("https://docs.zapfetch.com", { limit: 3 });
  ...
  test("start crawl with options", async () => {
    if (!client) throw new Error();
    const job = await client.startCrawl("https://docs.zapfetch.com", { limit: 5, maxDiscoveryDepth: 2 });
    expect(typeof job.id).toBe("string");
    expect(typeof job.url).toBe("string");
  }, 90_000);
  ...
  test("start crawl with prompt", async () => {
    if (!client) throw new Error();
    const job = await client.startCrawl("https://zapfetch.com", { prompt: "Extract all blog posts", limit: 3 });
    expect(typeof job.id).toBe("string");
    expect(typeof job.url).toBe("string");
  }, 90_000);
  ...
  test("get crawl status", async () => {
    if (!client) throw new Error();
    const start = await client.startCrawl("https://docs.zapfetch.com", { limit: 3 });
  ...
  test("get active crawls with running crawl", async () => {
    if (!client) throw new Error();
    const start = await client.startCrawl("https://docs.zapfetch.com", { limit: 5 });
  ...
  test("crawl with wait", async () => {
    if (!client) throw new Error();
    const job = await client.crawl("https://docs.zapfetch.com", { limit: 3, maxDiscoveryDepth: 2, pollInterval: 1, timeout: 120 });
  ...
  test("crawl with prompt and wait", async () => {
    if (!client) throw new Error();
    const job = await client.crawl("https://docs.zapfetch.com", { prompt: "Extract all blog posts", limit: 3, pollInterval: 1, timeout: 120 });
    expect(["completed", "failed"]).toContain(job.status);
    expect(job.completed).toBeGreaterThanOrEqual(0);
    expect(job.total).toBeGreaterThanOrEqual(0);
    expect(Array.isArray(job.data)).toBe(true);
  ...
  test("crawl with scrape options", async () => {
    if (!client) throw new Error();
    const job = await client.startCrawl("https://docs.zapfetch.com", {
      limit: 2,
  ...
  test("crawl params preview", async () => {
    if (!client) throw new Error();
    const params = await client.crawlParamsPreview("https://docs.zapfetch.com", "Extract all blog posts and documentation");
