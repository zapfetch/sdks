...
describe("v2.scrape e2e", () => {
  ...
  test("minimal: scrape only required params", async () => {
    ...
    const doc = await client.scrape("https://docs.zapfetch.com");
  ...
  test("maximal: scrape with all options", async () => {
    ...
    const doc = await client.scrape("https://docs.zapfetch.com", {
      ...
      parsers: ["pdf"],
      headers: { "User-Agent": "zapfetch-tests" },
      includeTags: ["article"],
      excludeTags: ["nav"],
  ...
  test("summary format returns summary string", async () => {
    if (!client) throw new Error();
    const doc = await client.scrape("https://zapfetch.com", { formats: ["summary"] });
  ...
  test.each([
    ...
    if (!client) throw new Error();
    const doc = await client.scrape("https://docs.zapfetch.com", { formats: [fmt as any] });
  ...
  test("images format: extract all images from webpage", async () => {
    if (!client) throw new Error();
    const doc = await client.scrape("https://zapfetch.com", {
      formats: ["images"],
    });
    expect(doc.images).toBeTruthy();
    expect(Array.isArray(doc.images)).toBe(true);
    expect(doc.images?.length).toBeGreaterThan(0);
    // Should find zapfetch logo/branding images
    expect(doc.images?.some(img => img.includes("zapfetch") || img.includes("logo"))).toBe(true);
  }, 60_000);
