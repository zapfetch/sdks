...
describe("v2.search e2e", () => {
  ...
  test("with sources web+news and limit", async () => {
    if (!client) throw new Error();
    const results = await client.search("zapfetch", { sources: ["web", "news"], limit: 3 });
    expect(results).toBeTruthy();
    expect(results.web).toBeTruthy();
    ...
    expect(allWebText.includes("zapfetch")).toBe(true);
  }, 90_000);
  ...
  test("with json format object", async () => {
    ...
    const results = await client.search("site:docs.zapfetch.com", {
      limit: 1,
      scrapeOptions: {
  ...
  test("with summary format, documents include summary when present", async () => {
    if (!client) throw new Error();
    const results = await client.search("site:zapfetch.com", {
      limit: 1,
