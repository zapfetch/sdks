import ZapfetchApp, { type CrawlParams, type CrawlResponse, type CrawlStatusResponse, type MapResponse, type ScrapeResponse } from '../../../index';
...
const API_URL = process.env.API_URL ?? "https://api.zapfetch.com";

describe('ZapfetchApp E2E Tests', () => {
  test.concurrent('should throw error for no API key only for cloud service', async () => {
    if (API_URL.includes('api.zapfetch.com')) {
      // Should throw for cloud service
      expect(() => {
        new ZapfetchApp({ apiKey: null, apiUrl: API_URL });
  ...
  test.concurrent('should throw error for invalid API key on scrape', async () => {
    if (API_URL.includes('api.zapfetch.com')) {
      const invalidApp = new ZapfetchApp({ apiKey: "invalid_api_key", apiUrl: API_URL });
  ...
  test.concurrent('should throw error for unsupported URL on scrape', async () => {
    const app = new ZapfetchApp({ apiKey: TEST_API_KEY, apiUrl: API_URL });
  ...
  test.concurrent('should check crawl status', async () => {
    const app = new ZapfetchApp({ apiKey: TEST_API_KEY, apiUrl: API_URL });
    const response = await app.asyncCrawlUrl('https://zapfetch.com', { limit: 20, scrapeOptions: { formats: ['markdown', 'html', 'rawHtml', 'screenshot', 'links'] } } as CrawlParams) as CrawlResponse;
    expect(response).not.toBeNull();
    expect(response.id).toBeDefined();
  ...
  test.concurrent('should return successful response for valid map', async () => {
    const app = new ZapfetchApp({ apiKey: TEST_API_KEY, apiUrl: API_URL }); const response = await app.mapUrl('https://roastmywebsite.ai') as MapResponse;
    expect(response).not.toBeNull();
  ...
  test('should search with string query', async () => {
    const app = new ZapfetchApp({ apiUrl: API_URL, apiKey: TEST_API_KEY });
    const response = await app.search("zapfetch");
  ...
  test('should handle invalid API key for search', async () => {
    const app = new ZapfetchApp({ apiUrl: API_URL, apiKey: "invalid_api_key" });
