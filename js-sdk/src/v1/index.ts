...
/**
 * Configuration interface for ZapfetchApp.
 * @param apiKey - Optional API key for authentication.
 * @param apiUrl - Optional base URL of the API; defaults to 'https://api.zapfetch.com'.
 */
export interface ZapfetchAppConfig {
  apiKey?: string | null;
  apiUrl?: string | null;
}

/**
 * Metadata for a Zapfetch document.
 * Includes various optional properties for document metadata.
 */
export interface ZapfetchDocumentMetadata {
  title?: string;
  description?: string;
  language?: string;
  keywords?: string;
...
/**
 * Document interface for Zapfetch.
 * Represents a document retrieved or processed by Zapfetch.
 */
export interface ZapfetchDocument<T = any, ActionsSchema extends (ActionsResult | never) = never> {
  url?: string;
  markdown?: string;
  html?: string;
  rawHtml?: string;
  ...
  json?: T;
  screenshot?: string;
  metadata?: ZapfetchDocumentMetadata;
 ...
 */
export interface ScrapeResponse<LLMResult = any, ActionsSchema extends (ActionsResult | never) = never> extends ZapfetchDocument<LLMResult, ActionsSchema> {
  success: true;
  warning?: string;
  error?: string;
}
...
export interface CrawlStatusResponse {
  ...
  expiresAt: Date;
  next?: string;
  data: ZapfetchDocument<undefined>[];
};
...
/**
 * Custom error class for Zapfetch.
...
/**
 * Main class for interacting with the Zapfetch API.
 * Provides methods for scraping, searching, crawling, and mapping web content.
 */
export default class ZapfetchApp {
  ...
  /**
   * Initializes a new instance of the ZapfetchApp class.
   * @param config - Configuration options for the ZapfetchApp instance.
   */
  constructor({ apiKey = null, apiUrl = null }: ZapfetchAppConfig) {
    const baseUrl = apiUrl || "https://api.zapfetch.com";
    
    if (this.isCloudService(baseUrl) && typeof apiKey !== "string") {
      throw new ZapfetchError("No API key provided", 401);
    }
  ...
  /**
   * Scrapes a URL using the Zapfetch API.
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (responseData.success) {
          ...
          throw new ZapfetchError(`Failed to scrape URL. Error: ${responseData.error}`, response.status);
        }
      } else {
  ...
  /**
   * Searches using the Zapfetch API and optionally scrapes the results.
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (responseData.success) {
          return {
            success: true,
            data: responseData.data as ZapfetchDocument<any>[],
            warning: responseData.warning,
          };
        } else {
          throw new ZapfetchError(`Failed to search. Error: ${responseData.error}`, response.status);
        }
      } else {
      ...
      if (error.response?.data?.error) {
        throw new ZapfetchError(`Request failed with status code ${error.response.status}. Error: ${error.response.data.error} ${error.response.data.details ? ` - ${JSON.stringify(error.response.data.details)}` : ''}`, error.response.status);
      } else {
        throw new ZapfetchError(error.message, 500);
      }
    }
  ...
  /**
   * Initiates a crawl job for a URL using the Zapfetch API.
  ...
  /**
   * Checks the status of a crawl job using the Zapfetch API.
  ...
  async checkCrawlStatus(id?: string, getAllData = false, nextURL?: string, skip?: number, limit?: number): Promise<CrawlStatusResponse | ErrorResponse> {
    if (!id) {
      throw new ZapfetchError("No crawl ID provided", 400);
    }
  ...
  /**
   * Cancels a crawl job using the Zapfetch API.
   * @param id - The ID of the crawl operation.
  ...
  async crawlUrlAndWatch(
    ...
    throw new ZapfetchError("Crawl job failed to start", 400);
  }

  /**
   * Maps a URL using the Zapfetch API.
  ...
  /**
   * Initiates a batch scrape job for multiple URLs using the Zapfetch API.
  ...
  async batchScrapeUrlsAndWatch(
    ...
    throw new ZapfetchError("Batch scrape job failed to start", 400);
  }

  /**
   * Checks the status of a batch scrape job using the Zapfetch API.
  ...
  async checkBatchScrapeStatus(id?: string, getAllData = false, nextURL?: string, skip?: number, limit?: number): Promise<BatchScrapeStatusResponse | ErrorResponse> {
    if (!id) {
      throw new ZapfetchError("No batch scrape ID provided", 400);
    }
  ...
  /**
   * Extracts information from URLs using the Zapfetch API.
  ...
  async extract<T extends zt.ZodSchema = any>(urls?: string[], params?: ExtractParams<T>): Promise<ExtractResponse<zt.infer<T>> | ErrorResponse> {
    ...
    try {
        ...
        do {
          ...
          if (extractStatus.status === "completed") {
            if (extractStatus.success) {
              ...
              throw new ZapfetchError(`Failed to extract data. Error: ${extractStatus.error}`, statusResponse.status);
            }
          } else if (extractStatus.status === "failed" || extractStatus.status === "cancelled") {
            throw new ZapfetchError(`Extract job ${extractStatus.status}. Error: ${extractStatus.error}`, statusResponse.status);
          }
      ...
      throw new ZapfetchError(error.message, 500, error.response?.data?.details);
    }
    return { success: false, error: "Internal server error."};
  }

  /**
   * Initiates an asynchronous extract job for a URL using the Zapfetch API.
  ...
  async monitorJobStatus(
    ...
    while (true) {
      try {
        ...
        if (statusResponse.status === 200) {
          ...
          if (statusData.status === "completed") {
            if ("data" in statusData) {
              ...
              throw new ZapfetchError("Crawl job completed but no data was returned", 500);
            }
          } else if (
        ...
        throw new ZapfetchError(error, 500);
      }
    }
  }
  ...
  async deepResearch(
    ...
    try {
      ...
      if (!response.id) {
        throw new ZapfetchError(`Failed to start research. No job ID returned.`, 500);
      }
  ...
  async checkDeepResearchStatus(id: string): Promise<DeepResearchStatusResponse | ErrorResponse> {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        throw new ZapfetchError("Deep research job not found", 404);
      } else {
  ...
  async generateLLMsText(url: string, params?: GenerateLLMsTextParams): Promise<GenerateLLMsTextStatusResponse | ErrorResponse> {
    try {
      ...
      if (!response.id) {
        throw new ZapfetchError(`Failed to start LLMs.txt generation. No job ID returned.`, 500);
      }
  ...
  async checkGenerateLLMsTextStatus(id: string): Promise<GenerateLLMsTextStatusResponse | ErrorResponse> {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        throw new ZapfetchError("LLMs.txt generation job not found", 404);
      } else {
...
interface CrawlWatcherEvents {
  document: CustomEvent<ZapfetchDocument<undefined>>,
  done: CustomEvent<{
...
export class CrawlWatcher extends TypedEventTarget<CrawlWatcherEvents> {
  ...
  public data: ZapfetchDocument<undefined>[];
  public status: CrawlStatusResponse["status"];
  public id: string;

  constructor(id: string, app: ZapfetchApp) {
    super();
    this.id = id;
