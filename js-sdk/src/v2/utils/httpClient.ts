  ...
  private async request<T = any>(config: AxiosRequestConfig): Promise<AxiosResponse<T>> {
    ...
    for (let attempt = 0; attempt < this.maxRetries; attempt++) {
      try {
        ...
        // For POST/PUT/PATCH, inject the SDK default origin ONLY when the
        // caller did not supply one. Preserves caller-specified origins
        // (e.g., attribution tags, routing metadata, integration markers).
        if (cfg.method && ["post", "put", "patch"].includes(cfg.method.toLowerCase())) {
          ...
          const callerOrigin =
            typeof data.origin === "string" && data.origin.length > 0
              ? data.origin
              : undefined;
          cfg.data = { ...data, origin: callerOrigin ?? `js-sdk@${version}` };
