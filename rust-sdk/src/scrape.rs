//! Scrape endpoint for ZapFetch API v2.
...
impl Client {
    ...
    ///
    /// ```no_run
    /// use zapfetch::{Client, ScrapeOptions, Format};
    ///
    /// #[tokio::main]
    ...
    pub async fn scrape(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| Error::HttpError(format!("Scraping {:?}", url.as_ref()), e))?;
    ...
    ) -> Result<ScrapeExecuteResponse, Error> {
    ...
    pub async fn stop_interaction(
        &self,
        job_id: impl AsRef<str>,
    ) -> Result<ScrapeBrowserDeleteResponse, Error> {
        let response = self
            .client
