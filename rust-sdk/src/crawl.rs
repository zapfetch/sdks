//! Crawl endpoint for ZapFetch API v2.
...
impl Client {
    ...
    ///
    /// ```no_run
    /// use zapfetch::{Client, CrawlOptions};
    ///
    /// #[tokio::main]
    ...
    /// }
    /// ```
    pub async fn get_crawl_status(&self, id: impl AsRef<str>) -> Result<CrawlJob, Error> {
        let response = self
            .client
    ...
    async fn get_crawl_status_next(&self, next: &str) -> Result<CrawlJob, Error> {
        let response = self
            .client
            .get(next)
            .headers(self.prepare_headers(None))
            .send()
            .await
            .map_err(|e| Error::HttpError(format!("Paginating crawl at {}", next), e))?;
    ...
    ///
    /// ```no_run
    /// use zapfetch::{Client, CrawlOptions, SitemapMode};
    ///
    /// #[tokio::main]
    /// async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ///     let client = Client::new("your-api-key")?;
    ...
    pub async fn get_crawl_errors(
        &self,
        id: impl AsRef<str>,
    ) -> Result<CrawlErrorsResponse, Error> {
        let response = self
            .client
        ...
        let mock = server
            ...
            .with_body(
                json!({
                    "success": true,
                    "id": "crawl-123",
                    "url": "https://api.zapfetch.com/v2/crawl/crawl-123"
                })
                .to_string(),
            )
            .create();
        ...
        let start_mock = server
            ...
            .with_body(
                json!({
                    "success": true,
                    "id": "crawl-456",
                    "url": "https://api.zapfetch.com/v2/crawl/crawl-456"
                })
                .to_string(),
            )
            .create();
