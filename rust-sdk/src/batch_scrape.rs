//! Batch scrape endpoint for ZapFetch API v2.
...
impl Client {
    ...
    ///
    /// ```no_run
    /// use zapfetch::{Client, BatchScrapeOptions};
    ///
    /// #[tokio::main]
    ...
    pub async fn start_batch_scrape(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| Error::HttpError("Starting batch scrape".to_string(), e))?;
    ...
    ///
    /// ```no_run
    /// use zapfetch::{Client, BatchScrapeOptions, ScrapeOptions, Format};
    ///
    /// #[tokio::main]
    /// async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ///     let client = Client::new("your-api-key")?;
    ...
    pub async fn get_batch_scrape_errors(
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                Error::HttpError(format!("Getting batch scrape errors {}", id.as_ref()), e)
            })?;
        ...
        let mock = server
            ...
            .with_body(
                json!({
                    "success": true,
                    "id": "batch-123",
                    "url": "https://api.zapfetch.com/v2/batch/scrape/batch-123"
                })
                .to_string(),
            )
            .create();
        ...
        let mock = server
            ...
            .with_body(
                json!({
                    "success": true,
                    "id": "batch-456",
                    "url": "https://api.zapfetch.com/v2/batch/scrape/batch-456",
        ...
        let start_mock = server
            ...
            .with_body(
                json!({
                    "success": true,
                    "id": "batch-789",
                    "url": "https://api.zapfetch.com/v2/batch/scrape/batch-789"
                })
                .to_string(),
            )
            .create();
