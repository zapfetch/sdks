//! Agent endpoint for ZapFetch API v2.
//!
...
impl Client {
    ...
    ///
    /// ```no_run
    /// use zapfetch::{Client, AgentOptions};
    ///
    /// #[tokio::main]
    ...
    pub async fn start_agent(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| Error::HttpError("Starting agent task".to_string(), e))?;
    ...
    ///
    /// ```no_run
    /// use zapfetch::{Client, AgentOptions, AgentModel};
    /// use serde_json::json;
    ///
    /// #[tokio::main]
    /// async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    /// }
    /// ```
    pub async fn cancel_agent(&self, id: impl AsRef<str>) -> Result<bool, Error> {
        let response = self
            .client
