 ...
 * 
 * Run with: ZAPFETCH_API_KEY=fc-xxx gradle test --tests "com.zapfetch.SearchTest"
 */
class SearchTest {
    ...
    void testSearchMinimal() {
        ...
        SearchData results = client.search("What is Zapfetch?");
    ...
    void testSearchWithMultipleSources() {
        ...
        SearchData results = client.search("Zapfetch web scraping",
                SearchOptions.builder()
    ...
    void testSearchWithScrapeOptions() {
        ...
        SearchData results = client.search("Zapfetch documentation",
                SearchOptions.builder()
                        .limit(2)
    ...
    @Test
    @EnabledIfEnvironmentVariable(named = "ZAPFETCH_API_KEY", matches = ".*\\S.*")
    void testSearchZapfetchSpecific() {
        System.out.println("\n=== Test: Search for Zapfetch ===");
        
        SearchData results = client.search("Zapfetch web scraping API",
                SearchOptions.builder()
                        .limit(10)
                        .build());

        assertNotNull(results.getWeb(), "Web results should not be null");
        assertFalse(results.getWeb().isEmpty(), "Should find Zapfetch results");
        
        // Verify results contain Zapfetch-related content
        boolean hasZapfetchContent = results.getWeb().stream()
                .anyMatch(result -> {
                    ...
                    return url.contains("zapfetch") ||
                           title.contains("zapfetch") ||
                           desc.contains("zapfetch");
                });
        
        assertTrue(hasZapfetchContent, "Results should mention Zapfetch");
        
        System.out.println("✓ Zapfetch search completed");
        System.out.println("  Total results: " + results.getWeb().size());
        System.out.println("  Results mentioning Zapfetch: ✓");
