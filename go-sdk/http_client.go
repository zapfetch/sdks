...
const (
	defaultAPIURL        = "https://api.zapfetch.com"
	defaultTimeout       = 5 * time.Minute
	defaultMaxRetries    = 3
	defaultBackoffFactor = 0.5
)

// httpClient is the internal HTTP client for the ZapFetch API.
...
func (h *httpClient) doJSON(ctx context.Context, method, url string, body interface{}, extraHeaders map[string]string) (json.RawMessage, error) {
	...
	if body != nil {
		...
		if err != nil {
			return nil, &APIError{Message: fmt.Sprintf("failed to serialize request body: %v", err)}
		}
	...
	for attempt := 0; attempt <= h.maxRetries; attempt++ {
		...
		req.Header.Set("User-Agent", "zapfetch-go/"+Version)
		...
		case 401:
			return nil, &AuthenticationError{
				APIError: APIError{StatusCode: 401, ErrorCode: errCode, Message: errMsg},
			}
		case 429:
			return nil, &RateLimitError{
				APIError: APIError{StatusCode: 429, ErrorCode: errCode, Message: errMsg},
			}
		}

		if resp.StatusCode >= 400 && resp.StatusCode < 500 && resp.StatusCode != 408 && resp.StatusCode != 409 {
			return nil, &APIError{StatusCode: resp.StatusCode, ErrorCode: errCode, Message: errMsg}
		}

		// Retryable: 408, 409, 5xx
		lastErr = &APIError{StatusCode: resp.StatusCode, ErrorCode: errCode, Message: errMsg}
	}

	if lastErr != nil {
		if fe, ok := lastErr.(*APIError); ok {
			return nil, fe
		}
		return nil, &APIError{Message: fmt.Sprintf("request failed after %d retries: %v", h.maxRetries, lastErr)}
	}
	return nil, &APIError{Message: "request failed"}
}
